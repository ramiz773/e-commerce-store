...
const AnalyticsTab = ({}) => {
  const [analyticsData, setAnalyticsData] = useState({ users: 0, products: 0, totalSale: 0, totalRevenue: 0 });
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-6 ">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <AnalyticsCard title="Total Sales" value={analyticsData.totalSale.toLocaleString()} icon={ShoppingCart} color="from=emerald-500 to-cyan-700" />
...
const AnalyticsCard = ({ title, value, icon: Icon, color }) => (
  <motion.div
    className={`bg-gray-800/60 rounded-lg p-6 shadow-lg overflow-hidden relative ${color}`}
    initial={{ opacity: 0, y: 20 }}
    animate={{ opacity: 1, y: 0 }}
